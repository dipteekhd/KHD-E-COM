export const LOAD_PRODUCTS = 'LOAD_PRODUCTS';

export const FILTER_BY_CATEGORY = 'FILTER_BY_CATEGORY';
export const FILTER_BY_RATING = 'FILTER_BY_RATING';
export const SORT_BY_PRICE = 'SORT_BY_PRICE';
export const FILTER_BY_PRICE_RANGE = 'FILTER_BY_PRICE_RANGE';
export const CLEAR_ALL_FILTER = 'CLEAR_ALL_FILTER';

export const INITIALIZE_AUTH = 'INITIALIZE_AUTH';
export const LOG_IN_SUCCESS = 'LOG_IN_SUCCESS';
export const LOG_OUT_SUCCESS = 'LOG_OUT_SUCCESS';

export const INCREMENT_WISH_COUNT = 'INCREMENT_WISH_COUNT';
export const DECREMENT_WISH_COUNT = 'DECREMENT_WISH_COUNT';
export const INITIALIZE_WISH_COUNT = 'INITIALIZE_WISH_COUNT';

export const INCREMENT_CART_COUNT = 'INCREMENT_CART_COUNT';
export const DECREMENT_CART_COUNT = 'DECREMENT_CART_COUNT';
export const INITIALIZE_CART_COUNT = 'INITIALIZE_CART_COUNT';
